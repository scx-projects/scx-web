...
final class ScxWebRouteImpl implements ScxWebRoute, Function1Void<RoutingContext, Throwable> {
    ...
    private static Object[] buildMethodParameters(ParameterHandler[] parameterHandlers, RoutingContext context, ScxWeb scxWeb) throws Exception {
        // 1, 获取 RequestInfo, 如果没有则创建 并 存储到 context.data 中. 这样同一个 RoutingContext 链上的 route 就可以共享 同一个 RequestInfo
        var requestInfo = (RequestInfo) context.data().computeIfAbsent("SCX_WEB_REQUEST_INFO", (_) -> new RequestInfo(context, scxWeb));
        // 2, 构建 方法参数.
        var methodParameter = new Object[parameterHandlers.length];

        for (int i = 0; i < methodParameter.length; i = i + 1) {
            methodParameter[i] = parameterHandlers[i].handle(requestInfo, scxWeb);
        }
    ...
    private void accept0(RoutingContext context) throws Throwable {
        ...
        var methodParameters = buildMethodParameters(parameterHandlers, context, this.scxWeb);
