...
/// ScxWeb
///
/// ScxWeb 只维护一个 [Interceptor] 槽位.
///
/// 这不是限制只能有一个拦截逻辑,
/// 而是规定 ScxWeb 只接受一个统一的拦截入口.
///
/// 多个拦截步骤应由 Interceptor 实现自行组合.
/// ScxWeb 不维护 Interceptor 列表,
/// 因为列表意味着框架需要定义顺序、短路、异常传播和 postHandle 回放规则.
///
/// 这些规则属于拦截器组合语义,
/// 不属于 ScxWeb 的路由注册语义.
///
...
public final class ScxWeb {
    ...
    public ScxWeb() {
        ...
        // 初始化序列化配置
        this.serializeConfig = SerializeConfig.of();
    ...
    /// 获取当前线程的 RoutingContext (只限在 scx-web 路由方法及其调用链上)
    ///
    ...
    /// 编译路由.
    ///
    /// 只有注册类自身直接声明的 [dev.scx.web.annotation.Route] 方法 会被暴露为路由;
    ///
    /// 详见 [RouteCompiler].
    ...
    public ScxWeb serializeConfig(SerializeConfig config) {
        this.serializeConfig = SerializeConfig.copyOf(config);
        return this;
    }
