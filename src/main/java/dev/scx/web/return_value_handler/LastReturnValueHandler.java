...
public final class LastReturnValueHandler implements ReturnValueHandler {
    ...
    public void handle(Object returnValue, ScxHttpServerRequest request, ScxWeb scxWeb) {
        ...
        if (contentType == MediaType.APPLICATION_JSON) {
            request.response().contentType(contentType).send(toJson(returnValue, scxWeb.serializeConfig()));
        } else {
            request.response().contentType(contentType).send(toXml(returnValue, scxWeb.serializeConfig()));
        }
    }
