...
public final class PathCaptureParameterHandler implements ParameterHandler {
    ...
    public Object handle(RequestInfo requestInfo, ScxWeb scxWeb) throws Exception {
        ...
        try {
            return ScxSerialize.nodeToObject(node, parameter.parameterType(), scxWeb.serializeConfig());
        } catch (NodeToObjectException e) {
            // 转换为 参数转换错误.
            throw new ParamConvertException("类型转换失败: @PathCapture " + captureName + " -> " + parameter.parameterType(), e);
        }
