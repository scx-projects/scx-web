...
public final class BodyFieldParameterHandler implements ParameterHandler {
    ...
    public Object handle(RequestInfo requestInfo, ScxWeb scxWeb) throws Exception {
        ...
        try {
            ...
            // 转换为 参数转换错误.
            throw new ParamConvertException("类型转换失败: @BodyField " + fieldName + " -> " + parameter.parameterType(), e);
        }
        if (value == null && bodyField.required()) {
            throw new RequiredParamMissingException("缺少必填参数: @BodyField " + fieldName);
        }
        return value;
    }
